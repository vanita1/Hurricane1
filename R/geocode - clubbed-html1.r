Geocode_combined_html<-function(tidy_tracks,stormvalue,cname){
...
tidy_tracks1 = tidy_tracks1[!duplicated(tidy_tracks1[c('latitude', 'wind_speed')]),]
...
ggtitle(paste("STORM NAME = " ,unique(track_list1[[k]]$stormid), "AT", "LATITUDE =" ,unique(track_list1[[k]]$latitude))) +
scale_color_manual(name = "Wind speed (kts)", 
                   values = c("red", "orange", "yellow")) + 
    scale_fill_manual(name = "Wind speed (kts)", 
                      values = c("red", "orange", "yellow")))} 
}
,img.name = "new2.html",loop=FALSE,
verbose=FALSE,ani.width = 960,ani.height = 960)
